...
export class SearchScreen extends HTMLElement {
    ...
    _saveRecentSearch(item) {
        try {
            // Проверяем, нет ли уже такого фильма в истории
            const exists = this._recentSearches.find(movie => movie.id === item.id);
            if (!exists) {
                // Добавляем новый фильм в начало массива
                this._recentSearches.unshift(item);
                // Оставляем только последние 10 фильмов
                this._recentSearches = this._recentSearches.slice(0, 10);
                // Сохраняем в localStorage
                localStorage.setItem('recentSearches', JSON.stringify(this._recentSearches));
    ...
    renderResults() {
        ...
        if (!this._searchResults.length || this._searchResults.length === 0) {
